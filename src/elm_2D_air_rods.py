...
def train_ann(X_train, targets, f_name):
    # training ELM
    elm = pipeline.Pipeline([('rhl', RandomLayer(n_hidden=240, activation_func='multiquadric')),
...
if __name__ == '__main__':
    prefix = '/home/adriano/Projects/ANNDispersionRelation/ann_training/2d/triangular/isotropic/'
    phcs = ['air_rods']
    phc = phcs[0]
    polarization = 'tm'
    ...
    elm_path = prefix + phc + '/with_material/' + polarization + '/ann_model/'
    elm_files = ['elm_240_2D_air_rods_pc.pkl']
    
    elm_file = elm_files[0]
    elm_verbose = 1
    train = 0
    nr_target_output_attrs = 3
    nr_training_samples = 728
    output = []
    ...
    save_estimatives('original_2D_tm_air_rods_pc.csv', ds.targetsTesting)
    #save_estimatives('elm_240_2D_tm_air_rods_pc.csv', outputs)