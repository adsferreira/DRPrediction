...
def train_ann(X_train, targets, f_name):
    # training ELM
    elm = pipeline.Pipeline([('rhl', RandomLayer(n_hidden=110, activation_func='multiquadric')),
...
if __name__ == '__main__':
    ...
    elm_files = ['elm_110_spheres_cyl_veins_pc.pkl']
    
    mlp_file = elm_files[0]
    mlp_verbose = 1
    train = 0
    ...
    if train:
        ...
        while(te_mse > 3.5e-01):
            start_time = time.clock()
            elm, mse = train_ann(X_train, ds.targets, elm_path + mlp_file)
        ...
        elm = load_model(elm_path, mlp_file)
        start_time = time.clock()
        outputs = predict(elm, X_test)
    ...
    #save_estimatives('original_spheres_cyl_veins_original_pc.csv', ds.targetsTesting)
    #save_estimatives('elm_110_spheres_cyl_veins_pc.csv', outputs)