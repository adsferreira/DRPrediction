...
def load_patterns():
    # load pattern data
    dataSet = ds.DataSet('/home/adriano/Projects/ANNDispersionRelation/ann_training/2d/triangular/te/16_interpolated_points/')
    dataSet.read_csv_file('dr_tri_rods_te_pc_dataset.csv')  
    #print(len(dataSet.all_patterns[192:,:]))
    return dataSet

def set_patterns_test(dataSet):
    dataSet.split_inputs_and_targets(2)
...
def train_ann(X_train, targets):
    # training MLP
    mlp = MLPRegressor(hidden_layer_sizes=(13,13), activation='tanh', solver='lbfgs', max_iter=1500, tol=1e-10)
    ...
    # save model
    #joblib.dump(mlp, 'models/23_12_17/mlp/tri_rods_pc/te/mlp_XX_tri_rods_te_pc.pkl')
    
    return (mlp, tr_mse)
    
def load_model():
    mlp = joblib.load('models/23_12_17/mlp/tri_rods_pc/te/mlp_13_13_tri_rods_te_pc.pkl') 
    return mlp
...
def save_estimatives(outputs):
    with open('test_mlp_tri_rods_te_pc_outs.csv', 'wb') as myfile:
