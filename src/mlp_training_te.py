...
def get_loss_vc_epoch(X_train, targets):
    #mse = []
    mlp = MLPRegressor(hidden_layer_sizes=(18,18), activation='tanh', solver='adam', tol=1e-8, max_iter=288, warm_start=False)
...
def load_model():
    mlp = joblib.load('models/2018/12_01_18/mlp/te_pc/mlp_25_25_25_te_pc.pkl') 
    return mlp
...
def save_estimatives(outputs):
    with open('original_test_te_pc_outs.csv', 'wb') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        wr.writerows(outputs)

def save_loss_vs_epoch(loss_vec):
    with open('/home/adriano/Projects/ANNDispersionRelation/ann_training/2d/square/te/16_interpolated_points/loss_vec.csv', 'wb') as myfile:
        wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
