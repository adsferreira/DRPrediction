...
def train_ann(X_train, targets, f_name):
    # training MLP
    mlp = MLPRegressor(hidden_layer_sizes=(10,10,10), activation='tanh', solver='lbfgs', max_iter=1500, tol=1e-8)
...
def get_loss_vc_epoch(X_train, targets):
    #mse = []
    mlp = MLPRegressor(hidden_layer_sizes=(16,16,16), activation='tanh', solver='adam', tol=1e-20, max_iter=716, warm_start=False)
...
if __name__ == '__main__':
    ...
    phcs = ['diamond3', 'diamond']
    phc = phcs[0]#[:-1]
    polarization = ''
    
    ds_path = prefix + phcs[0] + '/no_material' + polarization + '/ds/16_interpolated_points/'
    #ds_path = prefix + phcs[0] + '/with_material' + polarization + '/ds/16_interpolated_points/'
    ds_file = phc + '_' + polarization + 'ds.csv'
    #mlp_path = prefix + phcs[0] + '/with_material' + polarization + '/ann_models/'
    mlp_path = prefix + phcs[0] + '/no_material' + polarization + '/ann_models/'
    mlp_files = ['mlp_16_16_16_diamond3.pkl']
    
    mlp_file = mlp_files[0]
    mlp_verbose = 1
    train = 0
    nr_target_output_attrs = 3
    nr_training_samples = 618
    output = []
...
#             plot_results(ds.targetsTesting, outputs)
    ...
    #save_estimatives('original_test_diamond_pc.csv', ds.targetsTesting)
    #save_estimatives('mlp_10_10_10_diamond_pc.csv', outputs)