...
def train_ann(X_train, targets, f_name):
    # training MLP
    mlp = MLPRegressor(hidden_layer_sizes=(35,35), activation='tanh', solver='lbfgs', max_iter=1500, tol=1e-12)
...
def get_loss_vc_epoch(X_train, targets):
    #mse = []
    mlp = MLPRegressor(hidden_layer_sizes=(35,35), activation='tanh', solver='adam', tol=1e-20, max_iter=716, warm_start=False)
...
if __name__ == '__main__':
    prefix = '/home/adriano/Projects/ANNDispersionRelation/ann_training/3d/sc/'
    phcs = ['spheres_cylindrical_veins']
    phc = phcs[0]
    polarization = ''
    ...
    mlp_files = ['mlp_35_35_spheres_cyl_veins_pc.pkl']
    
    mlp_file = mlp_files[0]
    mlp_verbose = 1
    train = 0
    nr_target_output_attrs = 6
    nr_training_samples = 966
    output = []
...
#    save_estimatives('original_spheres_cyl_veins_pc.csv', ds.targetsTesting)
#    save_estimatives('mlp_35_35_spheres_cyl_veins_pc.csv', outputs)